 ...
 */
package com.framework.modules.gen.entity;
...
public class GenTable extends DataEntity<GenTable> {
	...
	public List<String> getImportGridJavaList() {
		...
		final Iterator<GenTableColumn> iterator = this.getColumnList().iterator();
		while (iterator.hasNext()) {
			final GenTableColumn column;
			if ((column = iterator.next()).getTableName() != null && !column.getTableName().equals("") && StringUtils.indexOf(column.getJavaType(),".") != -1 && !importList.contains(column.getJavaType())) {
				importList.add(column.getJavaType());
			}
		}
		return importList;
