...
<head>
	...
	<script type="text/javascript">
		...
		function createMenu(){
			...
			openDialog('创建菜单','/a/gen/genScheme/menuForm?gen_table_id='+id,'800px', '500px');
		}
		function page(n,s){
			if(n) $("#pageNo").val(n);
			if(s) $("#pageSize").val(s);
...
<body class="gray-bg">
	...
	<form:form id="searchForm" modelAttribute="genTable" action="${ctx}/gen/genTable/" method="post" class="form-inline">
		...
		<div class="form-group">
			<span>名称：</span><form:input path="nameLike" htmlEscape="false" maxlength="50"  class=" form-control input-sm"/>
	...
	<div class="col-sm-12">
			...
			<div class="pull-left">
				...
				<script type="text/javascript">
					function add(){
						openDialog("新增"+'表单',"/a/gen/genTable/form","80%", "80%","");
					}
				</script><!-- 增加按钮 -->
				</shiro:hasPermission>

				<shiro:hasPermission name="gen:genTable:synchDb">
				<button class="btn btn-white btn-sm " data-toggle="tooltip" data-placement="left" title="导入" onclick="openDialog('导入表单','/a/gen/genTable/importTableFromDB','700px', '300px')"><i class="fa fa-folder-open-o"></i> 数据库导入表单</button>
                </shiro:hasPermission>
				...
				<script type="text/javascript">
					...
					function edit(){
						...
						var id =  $("#contentTable tbody tr td input.i-checks:checkbox:checked").attr("id");
						openDialog("修改"+'表单',"/a/gen/genTable/form?id="+id,"80%", "80%","");
					}
				</script><!-- 编辑按钮 -->
				</shiro:hasPermission>
	...
	<table id="contentTable" class="table table-striped table-bordered table-hover table-condensed dataTables-example dataTable">
		...
		<c:forEach items="${page.list}" var="genTable">
			<tr>
				...
				<td><a  href="#" onclick="openDialogView('查看表单', '${ctx}/gen/genTable/form?id=${genTable.id}','800px', '500px')">
						${genTable.name}
				</a></td>
				<td>${genTable.comments}</td>
				<td>${genTable.className}</td>
				<td title="点击查询子表"><a href="javascript:" onclick="$('#parentTable').val('');$('#searchForm').submit();">${genTable.parentTable}</a></td>
				<td ${genTable.isSync == '0'?'style="background-color:red"':''}>${genTable.isSync == '0'?'<font color=\'white\'>未同步</font>':'已同步'}</td>
				<td>
					<shiro:hasPermission name="gen:genTable:edit">
					<a href="#" onclick="openDialog('修改业务表', '/a/gen/genTable/form?id=${genTable.id}','80%', '80%')" class="btn btn-success btn-xs" ><i class="fa fa-edit"></i> 修改</a>
                    </shiro:hasPermission>
